...
public class MersenneTwister {
    ...
    public init(seedBuffer: Array<UInt64>) {
        let arrayLength = seedBuffer.count
        mt = Array<UInt64>(repeating: UInt64(0), count: NN)
        mt[0] = 19650218
        mti = 1
        while mti < NN {
    ...
    public convenience init() {
        var bytes = Array<UInt8>(repeating: 0, count: 2496)

        // NOTE: Update this when Swift gets ported to other platforms
        let inputStream = FileHandle(forReadingAtPath: "/dev/random")
        ...
        let results = inputStream!.readData(ofLength: bytes.count)
        bytes = results.bytes.unsafeLoad(as: [UInt8].self)
        inputStream!.closeFile()
        var devRandBuffer = [UInt64](repeating: 0, count: bytes.count / 8)
        var counter = 0
        for b in bytes {
            let index = counter / 8
            counter += 1
    ...
    public func unsignedInt64() -> UInt64 {
        var i: Int
        var x: UInt64
    ...
    public func signedInt64() -> Int64 {
        return Int64(unsignedInt64() >> 1)
    }

    // generates a random number on [0,1]-real-interval
    public func u0_1_Inclusive() -> Scalar {
        return Scalar(unsignedInt64() >> 11) / 9007199254740991.0
    }

    // generates a random number on [0,1)-real-interval
    public func u0_1() -> Scalar {
        return Scalar(unsignedInt64() >> 11) / 9007199254740992.0
    }

    // generates a random number on (0,1)-real-interval
    public func u0_1_Exclusive() -> Scalar {
        return (Scalar(unsignedInt64() >> 12) + 0.5) / 4503599627370496.0
    }
}
